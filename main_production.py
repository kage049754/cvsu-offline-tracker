...
try:
    ...
    Logger.warning('Main: Android permissions not available (expected on desktop)')
...
def get_attendance_count(mode_name):
    """Get total attendance count for a mode."""
    db_path = StorageManager.get_database_path()
    conn = sqlite3.connect(db_path)
    cursor = conn.cursor()
    cursor.execute("SELECT COUNT(*) FROM activity_logs WHERE mode = ?", (mode_name,))
...
class CvSUSystemUI(BoxLayout):
    ...
    def __init__(self, **kwargs):
        ...
        # Button layout for check-in options
