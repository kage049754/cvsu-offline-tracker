...
class FreetypeRecipe(Recipe):
    """FreeType recipe with a stable SourceForge download URL."""
    ...
    def get_recipe_env(self, arch=None, with_harfbuzz=False):
        ...
        if with_harfbuzz:
            ...
            env['HARFBUZZ_LIBS'] = '-L{freetype}/lib -lfreetype -L{harfbuzz}/src/.libs -lharfbuzz'.format(
    ...
    def build_arch(self, arch, with_harfbuzz=False):
        ...
        if not harfbuzz_in_recipes:
            ...
            config_args.update({
                ...
                '--with-harfbuzz=no', '--with-zlib=yes',
            })
        elif not with_harfbuzz:
            info('Build freetype first time (without harfbuzz)')
            config_args.update({
                '--disable-shared', '--with-harfbuzz=no', '--with-zlib=no',
            })
        else:
            info('Build freetype second time (with harfbuzz)')
            config_args.update({
                '--disable-static', '--enable-shared',
                '--with-harfbuzz=yes', '--with-zlib=yes',
            })
        ...
        with current_directory(self.get_build_dir(arch.arch)):
            ...
            if not with_harfbuzz and harfbuzz_in_recipes:
                info('Installing freetype first build without harfbuzz')
